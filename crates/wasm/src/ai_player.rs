...
use crate::position::WASMPosition;

/// An enum to represent the difficulty of an AI player.
#[wasm_bindgen(js_name=Difficulty)]
#[derive(Copy, Clone, Debug)]
pub struct WASMDifficulty(Difficulty);

#[wasm_bindgen(js_class=Difficulty)]
impl WASMDifficulty {
    #[wasm_bindgen(getter, js_name=EASY)]
    pub fn easy() -> WASMDifficulty {
        WASMDifficulty(Difficulty::Easy)
    }

    #[wasm_bindgen(getter, js_name=MEDIUM)]
    pub fn medium() -> WASMDifficulty {
        WASMDifficulty(Difficulty::Medium)
    }

    #[wasm_bindgen(getter, js_name=HARD)]
    pub fn hard() -> WASMDifficulty {
        WASMDifficulty(Difficulty::Hard)
    }

    #[wasm_bindgen(getter, js_name=IMPOSSIBLE)]
    pub fn impossible() -> WASMDifficulty {
        WASMDifficulty(Difficulty::Impossible)
    }
}
...
#[wasm_bindgen(js_name=AIPlayer)]
#[derive(Debug)]
pub struct WASMAIPlayer(AIPlayer);

#[wasm_bindgen(js_class=AIPlayer)]
impl WASMAIPlayer {
    /// Creates a new AI player with a default solver and specified difficulty.
    #[wasm_bindgen(constructor)]
    pub fn new(difficulty: WASMDifficulty) -> WASMAIPlayer {
        WASMAIPlayer(AIPlayer::new(difficulty.0))
    }
    ...
    pub fn load_opening_book(&mut self, path: &str) -> bool {
    ...
    pub fn select_move(&self, position: &WASMPosition, scores: Vec<JsValue>) -> Option<usize> {
        ...
        let parsed_scores: Vec<Option<i8>> = scores
            .into_iter()
            .map(|val| {
                if val.is_null() || val.is_undefined() {
                    None
                } else {
                    val.as_f64().map(|num| num as i8)
                }
            })
            .collect();

        let scores_array: [Option<i8>; Position::WIDTH] = parsed_scores
            .try_into()
            .expect("Internal error converting scores vector to array.");
